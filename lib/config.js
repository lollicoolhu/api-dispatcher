...
let globalDelay = 0;
let pathDelays = {}; // { path: { delay, enabled } }
...
function loadData() {
  ...
  try {
    ...
    if (fs.existsSync(dataFile)) {
      ...
      if (data.globalDelay !== undefined) globalDelay = data.globalDelay;
      pathDelays = data.pathDelays || {};
...
module.exports = {
  ...
  setConfig: (config) => {
    ...
    if (config.globalDelay !== undefined) globalDelay = config.globalDelay;
    if (config.pathDelays !== undefined) pathDelays = config.pathDelays;
  },
